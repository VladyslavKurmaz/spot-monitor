...
class DeformableDetector(object):
    def __init__(self):
        ...
        self.arg_params, self.aux_params = load_param(cur_path + '/../model/' + ('rfcn_dcn_coco'), 0, process=True)
...
class DetectorWrapper(Thread):
    ...
    def run(self):
        while True:
            try:
                im, im_name = self.q_task.get(True, 1)
                result = self.detector.predict(im, im_name)
