...
def main():
    ...
    label_names = []
    data_shape_conf = [[('data', (1, 3, config.SCALES[0][0], config.SCALES[0][1])), ('im_info', (1L, 3L))]]
    data = [[mx.nd.array(data[i][name]) for name in data_names] for i in xrange(len(data))]
    pdb.set_trace()
    arg_params, aux_params = load_param(cur_path + '/../model/' + ('rfcn_dcn_coco'), 0, process=True)

    predictor = Predictor(sym, data_names, label_names,
                          context=[mx.gpu(0)], max_data_shapes=data_shape_conf,
                          provide_data=data_shape_conf, provide_label=[None],
                          arg_params=arg_params, aux_params=aux_params)
    nms = gpu_nms_wrapper(config.TEST.NMS, 0)

    # test
    for idx, im_name in enumerate(image_names):
        data_batch = mx.io.DataBatch(data=[data[idx]], label=[], pad=0, index=idx, provide_data=[[(k, v.shape) for k, v in zip(data_names, data[idx])]], provide_label=[None])
