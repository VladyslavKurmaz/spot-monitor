...
class SpotMonitor:
    ...
    def process(self, im):
        with self.lock:
            ...
            logger.debug(self.log + "Centers: {} Contours: {}".format(len(filtered_centers), len(cntrs)))
            ...
            logger.debug(self.log + "Suspicious region: {} ".format(len(susp_reg)))

            bbox_warped = self.warper.warp_image(img_shape=[im.shape[0], im.shape[1]],
                                                 coordinates=susp_reg, camera_id=1)

            logger.debug(self.log + "Warped suspicious region: {} ".format(len(bbox_warped)))
